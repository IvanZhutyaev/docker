...
<h4>Инструкция EXPOSE - позволяет задать внутренний порт контейнера</h4>
<h4>Инструкция VOLUME - позволяет указать место, которое контейнер будет использовать для постоянного хранения файлов и для работы с такими файлами</h4>
<h4>Инструкция FROM - сообщает Docker, о том, что при сборке образа использовался бы базовый образ, который соответствует предоставленному имени и тегу</h4>
